    ...
    this.getCategories = function(callback) {
        ...
        this.db.collection('item').aggregate(query)
            .toArray(function (err, categories){
                ...
                // assert.notEqual(category.length, 0);
                callback(categories);


            categories.forEach(function(category) {
                //console.log( category.name + " is a " + category.category_code + " company." );
...
/*this.db.collection("item").find({}).limit(4)toArray(function(err, relatedItems) {
       assert.equal(null, err);   callback(relatedItems); });*/
...
//            { $project: { _id: 1, title: 1, description: 1, slogan: 1, score: { $meta: 'textScore' } } },
            {$match: { score: { $gt: 1.0 } } },
            { $sort: { '_id': 1 } },
    ...
    this.getNumSearchItems = function(query, callback) {
        ...
        //console.log("This is the num Search Query " + numSearchQuery)
        var numItems = {};
           this.db.collection('item').find(numSearchQuery)
                .count(function(err, count){
                assert.equal(null, err);
                numItems = count;
                callback(numItems);
